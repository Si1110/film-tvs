...
def get_stoken(pwd_id, passcode=''):
    ...
    d = r.json()
    if d.get('status')==200: return d['data'].get('stoken','')
    return ''
...
for r in range(2, ws.max_row+1):
    ...
    if '致命弯道' in t:
        ...
        pwd_id, passcode = extract_pwd(lk)
        title = t
        print(f'Found: row{r} "{t}" pwd_id={pwd_id}')
        break
else:
    print('Not found!')
    sys.exit(1)
...
for e in entries:
    name = e['name']
    if any(w in name for w in promo_words): continue
...
# Save as dirs/致命弯道系列.html
dir_file = r'E:\workspace\github\film-tvs\res\dirs\致命弯道系列.html'
with open(dir_file, 'w', encoding='utf-8') as f:
    f.write(html)
print(f'Saved {dir_file} with {len(lines)} entries')

# Update Excel col8
ws.cell(r, 8).value = '致命弯道系列'
