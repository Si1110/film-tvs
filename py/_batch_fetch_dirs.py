...
def generate_baidu_placeholder(surl, title):
    return f"""<div class="text-center py-4">
  <p class="text-muted mb-3">该资源目录请前往百度网盘查看：</p>
  <a href="https://pan.baidu.com/s/{surl}" target="_blank" class="btn btn-warning px-4 py-2" style="font-weight:600;">
    打开百度网盘 <i class="bi bi-box-arrow-up-right"></i></a>
...
# Collect unique Quark pwd_ids and Baidu surls that need dir files
    ...
    for row in range(2, ws.max_row + 1):
        ...
        if m:
            ...
            if key not in existing_dirs:
                passcode = ''
                m2 = re.search(r'[?&]pwd=([^&]+)', c10)
                if m2: passcode = m2.group(1)
                if pwd_id not in quark_pending:
                    quark_pending[pwd_id] = passcode
        ...
        if m:
            ...
            if key not in existing_dirs:
                ...
                if surl not in baidu_pending:
                    baidu_pending[surl] = title

print(f'Need to fetch: {len(quark_pending)} Quark dirs, {len(baidu_pending)} Baidu placeholders')

# Process Quark entries
ok = 0
fail = 0
for pwd_id, passcode in quark_pending.items():
    print(f'  Quark [{pwd_id}] passcode=[{passcode}] ', end='', flush=True)
    try:
        ...
        for e in entries:
            ...
            lines.append(f'<div style="margin-left:{indent_px}px;color:{color};font-size:0.9rem;" class="mb-1">{icon} {e["name"]}</div>')
...
print(f'\nQuark done: {ok} ok, {fail} fail')

# Process Baidu entries (placeholders)
for surl, title in baidu_pending.items():
    html = generate_baidu_placeholder(surl, title)
    with open(os.path.join(DIRS_DIR, f'baidu_{surl}.html'), 'w', encoding='utf-8') as f:
        f.write(html)
print(f'Baidu placeholders: {len(baidu_pending)} generated')

# Second pass: update column 8 (目录路径) for ALL entries
...
wb.close()
print(f'Updated {updated_c8} entries in Excel')
print(f'\nAll done!')
